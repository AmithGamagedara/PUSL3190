    ...
    <div className="w-full bg-[#161616] py-4 sm:py-8">
      ...
      {data.map((card, index) => (
        <div key={index} className="m-4 mx-auto overflow-hidden bg-[#161616] shadow-md w-96 rounded-xl">
          <div className="p-4">
            <h2 className="mb-2 sm:text-2xl text-lg font-bold text-[#FFD600] sm:line-clamp-2">{card.title}</h2>
            <div className="flex justify-between mb-2">
              ...
              <span className="font-bold text-white text-[24px] sm:text-[32px]">{card.endDate}
              <span className="font-bold text-white text-[12px] sm:text-[20px]"> {card.endMonth}</span>
              </span>
            </div>
            <p className="mb-4 text-sm text-white sm:text-xl font-semi-bold">{card.location}</p>
